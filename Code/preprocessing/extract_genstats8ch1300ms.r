...
#  Time: 0-1300ms (0-260)
#  Data: mean, max, min, sd of each channel
#
...
# extract descriptive statistics from piece of signal
genstats <- function(signal) {
    return(c(min(signal), max(signal), mean(signal), sd(signal)))
}
...
extract <- function(dataset) {
    ...
    for (fbi in fb.idx) {
        counter <- counter + 1
        result <- rbind.data.frame(result, c(genstats(dataset[fbi:(fbi + 259), Cz]),
                                             genstats(dataset[fbi:(fbi + 259), FC6]),
                                             genstats(dataset[fbi:(fbi + 259), F5]),
                                             genstats(dataset[fbi:(fbi + 259), O2]),
                                             genstats(dataset[fbi:(fbi + 259), AF8]),
                                             genstats(dataset[fbi:(fbi + 259), CP6]),
                                             genstats(dataset[fbi:(fbi + 259), AF7]),
                                             genstats(dataset[fbi:(fbi + 259), CP5]),
                                             dataset$Subject[fbi]))
