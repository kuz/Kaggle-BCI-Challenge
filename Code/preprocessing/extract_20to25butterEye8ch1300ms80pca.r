...
# Butterworth filtering between 1 Hz to 3 Hz
prefilter <- function(signal){
  nyquist <- 100
  bf <- butter(1, c(20/nyquist, 25/nyquist), type="pass")
  z <- filter(bf, signal)
  return(z[-c(1:1000)])
}
...
extract <- function(dataset) {
  ...
  for (fbi in fb.idx) {
    counter <- counter + 1
    result <- rbind.data.frame(result, c(prefilter(dataset[(fbi-1000):(fbi + 259), Cz]), 
                                         prefilter(dataset[(fbi-1000):(fbi + 259), FC6]),
                                         prefilter(dataset[(fbi-1000):(fbi + 259), F5]),
                                         prefilter(dataset[(fbi-1000):(fbi + 259), O2]),
                                         prefilter(dataset[(fbi-1000):(fbi + 259), AF8]),
                                         prefilter(dataset[(fbi-1000):(fbi + 259), CP6]),
                                         prefilter(dataset[(fbi-1000):(fbi + 259), AF7]),
                                         prefilter(dataset[(fbi-1000):(fbi + 259), CP5]),
                                         dataset$Subject[fbi]))
...
folder = '20to25butterEye8ch1300ms80pca'
