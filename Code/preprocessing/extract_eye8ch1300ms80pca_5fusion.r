...
extract <- function(dataset) {
  ...
  if (dataset == 'train') {
    ...
    result <- cbind.data.frame(prediction.scores, subjects)
    setnames(result, c(paste('prediction_score_', methods, sep=''),'Subject'))
  } else if (dataset == 'test') {
    methods <- substr(files, start=35, stop=(nchar(files)-4))
    result <- prediction.scores
    setnames(result, paste('prediction_score_', methods, sep=''))   
  } else {
