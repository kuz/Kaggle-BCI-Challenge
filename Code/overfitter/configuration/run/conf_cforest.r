#
# Overfitter configuration for Conditional Inference Random Forest
#
...
# list of parameters
parameters <- list()
parameters[['mtry']] <- c(1, 10, 100, 400)
...
buildmodel <- function(p, trainingset) {
  tunegrid <- data.frame(mtry=p$mtry)
  trcontrol <- trainControl(method='none')
  classifier <- train(class ~., data=trainingset, 'cforest', trControl=trcontrol, tuneGrid=tunegrid)
  return(classifier)
}
